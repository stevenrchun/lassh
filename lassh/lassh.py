...
def checkFiles():
    ...
    if (not LASSH_CONFIG_PATH.exists()):
        puts(colored.red('lassh.config file not found in current directory, try `lassh init`'))
        return

def deleteInclude():
    # File line modification from: https://stackoverflow.com/questions/4710067/deleting-a-specific-line-in-a-file-python
    ...
    with open(HOME_SSH_CONFIG_PATH, 'r+') as f:
        ...
        for line in lines:
            ...
            if lineComponents and lineComponents[0]=='Include':
                ...
                # Check if path indicated by Include and current lassh.config are the same file
                if (os.path.samefile(LASSH_CONFIG_PATH.resolve(), includePath)):
                    puts(colored.red('Deleting include for {0}'.format(includePath)))
                    includeFound=True
    ...
    with open(HOME_SSH_CONFIG_PATH, 'r') as f:
        ...
        for line in lineSet:
            ...
            if lineComponents and lineComponents[0]=='Include':
               ...
               # Check if path indicated by Include and current lassh.config are the same file
               if (os.path.samefile(LASSH_CONFIG_PATH.resolve(), includePath)):
                   puts(colored.yellow('Global ssh_config already includes this file'))
                   return
...
@lassh.command()
@click.confirmation_option(prompt="Are you sure you want to unlink your project lassh.config?")
def teardown():
    "Deletes reference to lassh.config in current directory from global ssh_config"
    checkFiles()
...
def addhost(nickname, hostname, user, port, key):
    ...
    if (port and key):
        config.add(nickname, HostName=hostname, User=user, Port=port, IdentityFile=key)
    elif port:
    ...
    puts(colored.green('Adding host {0} as {1} with user {2}').format(hostname, nickname, user))
